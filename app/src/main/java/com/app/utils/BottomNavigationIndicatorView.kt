...
open class BottomNavigationIndicatorView @JvmOverloads constructor(
    ...
    private var index =
        0 //The current index selected of the BottomNav. This is also used to restore the view state.

    init {
        if (attributeSet == null) {
            targetId = NO_ID
            indicatorDrawable =
                ColorDrawable(ContextCompat.getColor(context, R.color.colorIndicator_indicatorView))
        } else {
            with(
                context.obtainStyledAttributes(
                    attributeSet,
                    R.styleable.BottomNavigationIndicatorView
                )
            ) {
                targetId = getResourceId(
                    R.styleable.BottomNavigationIndicatorView_targetBottomNavigation_indicatorView,
                    NO_ID
                )

                val backgroundId = getResourceId(
                    R.styleable.BottomNavigationIndicatorView_customIndicatorBackground_indicatorView,
                    NO_ID
                )
                indicatorDrawable = if (backgroundId == NO_ID) ColorDrawable(
                    ContextCompat.getColor(
                        context,
                        R.color.colorIndicator_indicatorView
                    )
                )
                else ContextCompat.getDrawable(context, backgroundId) ?: ColorDrawable(
                    ContextCompat.getColor(
                        context,
                        R.color.colorIndicator_indicatorView
                    )
    ...
    override fun onAttachedToWindow() {
        ...
        if (child !is BottomNavigationView) return attachedError("Invalid view child or could not find MultiListenerBottomNavigationView")
    ...
    open fun updateRectByIndex(index: Int, animated: Boolean) {
        this.index = index
        bottomNav?.apply {
            if (childCount < 1 || index >= childCount - 1) return
